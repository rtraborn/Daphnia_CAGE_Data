...
require(BSgenome.Dpulex.IU.dappu3)
...
thisDir <- c("/home/rtraborn/Daphnia/CAGE/PA42/demultiplexed_matched/bam_files")
...
myCAGEset <- new("CAGEset",genomeName="BSgenome.Dpulex.JGI.dpulex",inputFiles=pathsToInputFiles,inputFilesType="bam",sampleLabels=c("asex_rep1","asex_rep2","asex_rep3","males_rep1","males_rep2","sex_rep1","sex_rep2","sex_rep3"))

#calculating the CTSS for the dataset
getCTSS(myCAGEset)
...
save.image("Dp_PA42v3.RData")

#clustering the CTSS into so-called Tag Clusters (TCs), which we call TSRs (transcription start regions)
clusterCTSS(object = myCAGEset, threshold = 1, thresholdIsTpm = FALSE ,nrPassThreshold = 1, method = "distclu", maxDist = 20,removeSingletons = TRUE, keepSingletonsAbove = 5, useMulticore = T, nrCores = 6)
...
write.table(consensusCl,file="consensus_clusters_PA42v3_tpm.txt",row.names=FALSE,col.names=TRUE,quote=FALSE)
...
write.table(TSR_summary,file="TSR_interquantile_summary_PA42v3.txt",sep=" ",col.names=TRUE,row.names=FALSE)
...
save(myCAGEset,file="Dp_CAGEset_PA42v3.RData")
