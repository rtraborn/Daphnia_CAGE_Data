...
fig12_p + geom_tile() + scale_fill_gradient2(low="#D7191C", mid="white", high="#2C7BB6") +
                                        #   geom_text(aes(label=stars, color=value), size=8) + scale_colour_gradient(low="grey30", high="white", guide="none") +
geom <- text(aes(label=stars), color="black", size=5) +
    labs(y=NULL, x=NULL, fill="t-value") + geom <- vline(xintercept=1.5, size=1.5, color="grey50") +
    theme <- bw() + theme(axis.text.x=element <- text(angle = -45, hjust = 0))
fig12
