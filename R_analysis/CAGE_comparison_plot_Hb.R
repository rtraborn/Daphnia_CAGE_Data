...
coord_region <- GRanges("scaffold_13", IRanges(634000,636000),strand="-")

myFlag <- scanBamFlag()

my_param <- ScanBamParam(flag=myFlag,what=c("flag", "mrnm", "mpos"), which=GRanges("scaffold_13", IRanges(634000,636000),strand="-"))
...
myStrand <- GRanges("scaffold_13",IRanges(endDp,end(aln)),strand=strandDp,names=names(aln))
...
matfem_fem_param<- ScanBamParam(flag=myFlag,what=c("flag", "mrnm", "mpos"), which=GRanges("scaffold_13",IRanges(634000,636000),strand="-"))
...
myStrand <- GRanges("scaffold_13",IRanges(endDp,end(p_aln)),strand=strandDp,names=names(p_aln))
...
p1_p <- plot_peaked + theme_bw() + ylim(0,1000)
...
matmale_param <- ScanBamParam(flag=myFlag,what=c("flag", "mrnm", "mpos"), which=GRanges("scaffold_13",IRanges(634000,636000),strand="-"))
...
p3_p <- plot_peaked + theme_bw() + ylim(0,1000)
...
ggsave(file="Hb_upreg_pE_plot_combined.png", width=6, height=2, scale=1.5)
