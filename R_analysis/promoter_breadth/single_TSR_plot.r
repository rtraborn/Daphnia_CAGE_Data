...
Dp.bam <- c("/home/rtraborn/Daphnia/CAGE/Dp_CAGE_all_sorted.bam")

Dp.bai <- c("/home/rtraborn/Daphnia/CAGE/Dp_CAGE_all_sorted.bam.bai")
...
ggsave(file="broad_TSR.png", width=6, height=4.5,dpi=300,scale=1.5)

#scaffold_4:2,460,642-2,462,870

peaked_gr <- GRanges("scaffold_4", IRanges(2460951,2462370),strand="+")
...
p1_p <- plot_peaked + coord_cartesian(xlim = c(2460951,2462370),ylim=c(0,1000)) + theme_bw()

p2_p <- ggplot() + geom_alignment(gr_gene4) + coord_cartesian(xlim = c(2460951,2462370)) + scale_x_continuous() + 
                             theme_bw()
...
ggsave(file="peaked_TSR.png", width=6, height=4.5,dpi=300,scale=1.5)
